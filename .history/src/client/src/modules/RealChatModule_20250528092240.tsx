...
const RealChatModule: React.FC<RealChatModuleProps> = ({
  ...
  useEffect(() => {
    ...
    // Cargar mensajes iniciales cuando se selecciona un contacto
    const loadInitialMessages = async (contactId: string) => {
      try {
        const response = await fetch(`http://localhost:3001/messages?contactId=${contactId}`);
