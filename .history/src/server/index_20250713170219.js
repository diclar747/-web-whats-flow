...
async function initializeDatabase() {
    ...
    try {
        ...
        if (error.code === 'ECONNREFUSED') {
            console.error('[DB-INIT] Connection refused. Is MySQL server running and accessible?');
            console.error('[DB-INIT] 💡 TIP: Ejecuta con "SET SKIP_DB=true && node index.js" para funcionar sin DB');
        }
...
async function saveMessageToDB(sessionId, msg) {
    ...
    try {
        ...
        console.log(`[DB-MSG-QUERY] Attempting to insert/update messageId: ${messageId} for session: ${sessionId}, chat_jid: ${chat_jid}, sender_jid: ${finalSenderJid}, from_me: ${from_me}, type: ${params[5]}, status: ${params[10]}`);
        const [result] = await connection.execute(
            'INSERT INTO messages (id, session_id, chat_jid, sender_jid, from_me, message_type, text_content, media_url, media_mime_type, timestamp, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE status = VALUES(status), updated_at = CURRENT_TIMESTAMP',
            params
        );
        ...
        if (result.affectedRows > 0) {
             if (result.insertId !== 0 && result.insertId !== undefined) { // insertId is 0 for ON DUPLICATE KEY UPDATE if no new row was inserted
                console.log(`[DB-MSG] Message ${messageId} for session ${sessionId} inserted into DB.`);
             } else {
                console.log(`[DB-MSG] Message ${messageId} for session ${sessionId} updated in DB (e.g. status).`);
             }
        } else {
            ...
            console.log(`[DB-MSG] Message ${messageId} for session ${sessionId} - no change in DB (already exists with same status?).`);
        }
        return result;
...
async function getOrCreateUserSession(sessionId, phoneNumber) {
    ...
    try {
        ...
        await connection.query(
            'INSERT INTO user_sessions (session_id, phone_number, is_active) '
            + 'VALUES (?, ?, TRUE) '
            + 'ON DUPLICATE KEY UPDATE phone_number = VALUES(phone_number), is_active = TRUE, last_activity = CURRENT_TIMESTAMP',
            [sessionId, phoneNumber]
        );

        console.log(`[DB-USER] Sesión de usuario registrada: ${sessionId} -> ${phoneNumber}`);
        return phoneNumber;
    } catch (error) {
...
async function loadChatListFromDB(sessionId) {
    ...
    if (!userPhoneNumber) {
        console.log(`[DB-CHATLIST] No se pudo obtener el número de usuario para ${sessionId}`);
        return [];
    }

    const connection = await pool.getConnection();
    try {
        console.log(`[DB-CHATLIST] Loading chat list for session ${sessionId} and user ${userPhoneNumber}`);
        
        // Query modificada para filtrar por el número de teléfono del usuario
        // Buscar mensajes donde el usuario sea el remitente O el destinatario
        ...
        console.log(`[DB-CHATLIST] Loaded ${chatList.length} chats for user ${userPhoneNumber} in session ${sessionId}`);
        return chatList;
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    const userPhoneNumber = await getUserPhoneNumber(sessionId);
                    if (userPhoneNumber) {
                        await getOrCreateUserSession(sessionId, userPhoneNumber);
                        console.log(`[${sessionId}] Usuario registrado: ${userPhoneNumber}`);
                    }
                } catch (error) {
                ...
                // Cargar lista de chats inicial desde la DB y emitirla
                try {
            ...
            if (connection === 'close') {
                ...
                if (shouldReconnect) {
                    ...
                    console.log(`[${sessionId}] Usuario desconectado`);
                    sessions.delete(sessionId);
                }
            }
        });
...
app.get('/api/session/:sessionId/status', (req, res) => {
    ...
    res.json({
        ...
        message: session.isConnected ? 'Sesión conectada' : 'Sesión desconectada',
        sessionId: sessionId
    });
});
...
app.get('/api/messages/:sessionId', async (req, res) => {
    ...
    const connection = await pool.getConnection();
    try {
        let query = 'SELECT id, session_id, chat_jid, sender_jid, from_me, message_type, text_content, media_url, media_mime_type, timestamp, status FROM messages WHERE session_id = ?';
        ...
        const countQuery = query.replace('SELECT id, session_id, chat_jid, sender_jid, from_me, message_type, text_content, media_url, media_mime_type, timestamp, status', 'SELECT COUNT(*) as total');
