...
interface BotSettings { enabled: boolean; workingHours: { enabled: boolean; start: string; end: string; days: number[]; }; fallbackMessage: string; transferToAgent: boolean; aiEnabled: boolean; responseDelay: number; botMode: 'flows' | 'ai'; aiConfig?: { businessData: string; websiteUrl: string; scrapedContent: string; temperature: number; maxTokens: number; }; }
...
const ChatbotModuleContent: React.FC<ChatbotModuleProps> = ({ sessionId }) => {
  ...
  const [settings, setSettings] = useState<BotSettings>({ enabled: false, workingHours: { enabled: false, start: '09:00', end: '18:00', days: [1,2,3,4,5] }, fallbackMessage: 'Lo siento, no entiendo tu mensaje.', transferToAgent: true, aiEnabled: false, responseDelay: 1000, botMode: 'flows', aiConfig: { businessData: '', websiteUrl: '', scrapedContent: '', temperature: 0.7, maxTokens: 500 } });
          ...
          <CardContent>
            ...
            <Stack spacing={3} sx={{ mt: 3 }}>
              ...
              <FormControlLabel
                ...
                label={<Box><Typography variant="body1" fontWeight="bold">Habilitar Chatbot</Typography><Typography variant="caption" color="textSecondary">El bot responderá automáticamente</Typography></Box>}
              />
              ...
              {/* Selector de Modo */}
              <Box>
                <Typography variant="subtitle1" fontWeight="bold" gutterBottom>Modo de Operación</Typography>
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <Card
                      variant={settings.botMode === 'flows' ? 'elevation' : 'outlined'}
                      elevation={settings.botMode === 'flows' ? 8 : 0}
                      sx={{
                        cursor: 'pointer',
                        border: settings.botMode === 'flows' ? '2px solid #667eea' : '1px solid #e0e0e0',
                        transition: 'all 0.3s'
                      }}
                  ...
                  <Grid item xs={12} md={6}>
                    <Card
                      variant={settings.botMode === 'ai' ? 'elevation' : 'outlined'}
                      elevation={settings.botMode === 'ai' ? 8 : 0}
                      sx={{
                        cursor: 'pointer',
                        border: settings.botMode === 'ai' ? '2px solid #10b981' : '1px solid #e0e0e0',
                        transition: 'all 0.3s'
                      }}
                    ...
                    >
                      <CardContent>
                        ...
                        <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
                          Respuestas inteligentes generadas con DeepSeek AI
                        </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
              ...
              {/* Configuración de IA (solo visible si botMode === 'ai') */}
              {settings.botMode === 'ai' && (
                <Box>
                  <Alert severity="info" sx={{ mb: 2 }}>
                    🤖 Configura la información del negocio para que la IA pueda responder de manera personalizada
                  </Alert>
                  ...
                  <Stack spacing={2}>
                    ...
                    <Typography variant="subtitle2" gutterBottom>
                      O Extrae Datos desde tu Sitio Web
                    </Typography>
                    ...
                    {settings.aiConfig?.scrapedContent && (
                      <TextField
                        ...
                        multiline
                        rows={4}
                        value={settings.aiConfig.scrapedContent}
                        disabled
                        helperText="Contenido extraído automáticamente desde tu sitio web"
                      />
                    )}
                    ...
                    <Box>
                      <Typography variant="caption" color="textSecondary" gutterBottom>
                        Temperatura: {settings.aiConfig?.temperature || 0.7} (Creatividad de las respuestas)
                      </Typography>
                    ...
                    <TextField
                      ...
                      helperText="Longitud máxima de las respuestas (100-2000)"
                    />
                  </Stack>
                </Box>
              )}
              ...
              <Box>
                <TextField
                  ...
                  InputProps={{
                    endAdornment: <IconButton onClick={(e) => handleEmojiClick(e, 'fallback')} size="small"><EmojiEmotions/></IconButton>
                  }}
                  helperText="Mensaje cuando el bot no puede responder"
                />
              </Box>
