                    ...
                    >
                      <ListItemAvatar>
                        ...
                        >
                          ...
                          >
                            {(chat.name || (chat as any).pushName || chat.id || '').charAt(0).toUpperCase()}
                          </Avatar>
                        </Badge>
                      </ListItemAvatar>
                      <ListItemText
            ...
            {activeChat ? (
              <>
                ...
                <div style={{
                  ...
                  >
                    {(activeChat?.name || (activeChat as any)?.pushName || activeChat?.id || '').charAt(0).toUpperCase()}
                  </Avatar>
                  <div style={{ marginLeft: '15px', flex: 1 }}>
                    <div style={{ fontWeight: 'bold' }}>{activeChat?.name || (activeChat as any)?.pushName || normalizePhoneNumber(activeChat?.id || '')}</div>
                    <div style={{
                      fontSize: '0.8rem',
                      color: isDarkMode ? '#aebac1' : '#667781',
                    }}>
                  ...
                  {isLoading && messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      <div key={msg.id} style={{
                        display: 'flex',
                        justifyContent: msg.type === 'system' ? 'center' : (msg.isFromMe ? 'flex-end' : 'flex-start'),
                        marginBottom: '10px',
                        alignItems: 'flex-end',
                        maxWidth: msg.type === 'system' ? '100%' : '90%',
                        marginLeft: msg.type === 'system' ? '0' : (msg.isFromMe ? 'auto' : '10px'),
                        marginRight: msg.type === 'system' ? '0' : (!msg.isFromMe ? 'auto' : '10px'),
                        position: 'relative'
                      }}>
                        {msg.type === 'system' ? (
                          <Paper
                          ...
                          >
                            {msg.message || (msg as any).text_content}
                          </Paper>
                        ) : (
                          <>
                            {!msg.isFromMe && (
