...
const { sendPlanRequestEmail, sendPlanApprovalEmail } = require('../utils/subscriptionEmail');
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const { findAdminSessionId } = require('../utils/subscriptionNotification');
                    const adminSessionId = await findAdminSessionId();

                    await sendWhatsAppMessage(adminSessionId, phone, whatsappMessage).catch(err => {
                        console.log('[PLAN-REQUEST] ⚠️ No se pudo enviar mensaje WhatsApp al cliente desde', adminSessionId, ':', err.message);
                    });

                    console.log('[PLAN-REQUEST] ✅ Mensaje WhatsApp enviado al cliente:', phone, 'desde', adminSessionId);
                } catch (whatsappError) {
